...
if __name__ == '__main__':
    ...
    benchmark_filename = "benchmark/benchmark_results_multisearch_many_v_many.csv"
    ...
    for query_filelist, query_filesize in list(zip(filelists, filesizes)):
        for ref_filelist, ref_filesize in list(zip(filelists, filesizes)):
            ...
            output_filename = f"multisearch_{query_filesize}_v_{ref_filesize}_results.csv"
            output_filename = os.path.join(data_dir, output_filename)
            
            command = f"sourmash scripts multisearch -o {output_filename} -c 128 {query_filelist_path} {ref_filelist_path} -k 51"
            print(command)
